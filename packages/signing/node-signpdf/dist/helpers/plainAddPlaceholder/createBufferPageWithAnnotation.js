...
const createBufferPageWithAnnotation = (pdf, info, pagesRef, widget) => {
  const pagesDictionary = (0, _findObject.default)(
    pdf,
    info.xref,
    pagesRef
  ).toString(); // Extend page dictionary with newly created annotations
  ...
  const pagesDictionaryIndex = (0, _getIndexFromRef.default)(
    info.xref,
    pagesRef
  );
