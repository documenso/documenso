...
export const run = async ({
  ...
  let envelopeItems: typeof envelope.envelopeItems = await io.runTask(
    'get-document-data-id', // Todo: Envelopes - Fix these messed up types.
...
const decorateAndSignPdf = async ({
  ...
  await flattenForm(pdfDoc);
  console.log('END FLATTENING FORM', Date.now() - currentTime, 'ms');

  currentTime = Date.now();
  console.log('START SAVING');
  const pdfBytes = await pdfDoc.save();
  console.log('END SAVING', Date.now() - currentTime, 'ms');

  currentTime = Date.now();
  console.log('START SIGNING');
  const pdfBuffer = await signPdf({ pdf: Buffer.from(pdfBytes) });
  console.log('END SIGNING', Date.now() - currentTime, 'ms');
