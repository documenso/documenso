...
import {
  getAuditLogPdfByPlaywright as getAuditLogPdf,
  getCertificatePdfByPlaywright as getCertificatePdf,
} from '../../../server-only/htmltopdf/get-pdf-by-playwright';
...
export const getCertificateAndAuditLogData = async ({
  ...
  const getCertificateDataPromise = settings.includeSigningCertificate
    ? getCertificatePdf(payload).catch((e) => {
  ...
  const getAuditLogDataPromise = settings.includeAuditLog
    ? getAuditLogPdf(payload).catch((e) => {
