...
import type { TContentLineMeta, TEnvelopeContentMetaOutput } from '../../types/envelope-content-meta';
import {
  CONTENT_TYPE_DATA_CONTENT_KIND,
  EnvelopeContentType,
  isContentTypeRotatable,
...
export const resolveLineMetaFromPoints = (
  ...
  return {
    ...meta,
    startXPosition: toPercentage(groupX + points[0], pageWidth),
    startYPosition: toPercentage(groupY + points[1], pageHeight),
    endXPosition: toPercentage(groupX + points[2], pageWidth),
    endYPosition: toPercentage(groupY + points[3], pageHeight),
  };
};
...
export const resolveContentMetaFromTransform = (
  meta: TEnvelopeContentMetaOutput,
...
export const getContentTransformerConfig = (
  ...
  const isSingleRotatable = types.length === 1 && type !== undefined && isContentTypeRotatable(type);

  const isSingleImageWithImage = isSingleRotatable && CONTENT_TYPE_DATA_CONTENT_KIND[type] !== undefined && hasImage;
