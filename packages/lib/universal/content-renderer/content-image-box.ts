 ...
 */
export const CONTENT_IMAGE_MAX_PAGE_FRACTION = 0.8;

/**
 * How image pixels map to page points when auto sizing: one pixel is one
 * point at scale 1.
 */
export const CONTENT_IMAGE_PIXELS_PER_POINT = 1;
...
const autoSizeBox = ({ image, page, box }: ResolveAttachedImageBoxOptions): ContentBox => {
  ...
  const widthPercent = (width / page.width) * 100;
  const heightPercent = (height / page.height) * 100;
...
const tightenBox = ({ image, page, box }: ResolveAttachedImageBoxOptions): ContentBox => {
  const boxInPoints = {
    width: (box.width / 100) * page.width,
    height: (box.height / 100) * page.height,
  };

  const fit = calculateContainFit(image, boxInPoints);

  return {
    positionX: box.positionX + (fit.x / page.width) * 100,
    positionY: box.positionY + (fit.y / page.height) * 100,
    width: (fit.width / page.width) * 100,
    height: (fit.height / page.height) * 100,
  };
};
 ...
 */
const clampPosition = (position: number, extent: number) => {
  return Math.max(0, Math.min(position, 100 - extent));
};
