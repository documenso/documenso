...
import { CONTENT_IMAGE_DEFAULT_SIZE, isContentImageDefaultSize, resolveAttachedImageBox } from './content-image-box';
...
describe('isContentImageDefaultSize', () => {
  it('matches the drop default exactly', () => {
    expect(isContentImageDefaultSize(CONTENT_IMAGE_DEFAULT_SIZE)).toBe(true);
    expect(isContentImageDefaultSize({ width: CONTENT_IMAGE_DEFAULT_SIZE.width, height: 10.5 })).toBe(false);
  });
});
