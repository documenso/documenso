...
export function assertIsPDF(buf: Buffer): void {
  if (!isPDF(buf)) {
    throw new AppError(AppErrorCode.INVALID_BODY, {
      ...
      userMessage: 'The uploaded file is not a PDF. Only PDF files can be verified.',
      statusCode: 400,
    });
  }
}
...
const MAX_FILE_BYTES = parseInt(process.env.MAX_FILE_SIZE ?? '10485760', 10); // 10MB default
...
export function assertFileSize(buf: Buffer): void {
  if (buf.length > MAX_FILE_BYTES) {
    ...
    throw new AppError(AppErrorCode.INVALID_BODY, {
      ...
      userMessage: `File exceeds the maximum allowed size of ${maxMB}MB.`,
      statusCode: 400,
    });
  }
}
 ...
 *
 * Usage pattern (enforced in verify-document.ts):
 *
