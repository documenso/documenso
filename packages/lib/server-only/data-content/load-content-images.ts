...
export const loadContentImages = async (
  ...
  const dataContentIds = Array.from(
    new Set(contents.flatMap((content) => (content.dataContentId ? [content.dataContentId] : []))),
  );
  ...
  const entries = await Promise.all(
    dataContents.map(async (dataContent): Promise<[string, ContentImageSource] | null> => {
      ...
      return [dataContent.id, createSkiaImage(bytes)];
    }),
  );
