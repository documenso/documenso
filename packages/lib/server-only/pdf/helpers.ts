...
export const ensureFontLibrary = () => {
  const fontPath =
    resolvePdfPublicAssetPath('fonts/caveat.ttf')?.replace(/\/caveat\.ttf$/, '') ??
...
export const resolvePdfPublicAssetPath = (relativePath: string) => {
  const normalizedRelativePath = relativePath.replace(/^\/+/, '');

  return [
    path.join(process.cwd(), 'public', normalizedRelativePath),
    path.join(process.cwd(), 'apps/remix/public', normalizedRelativePath),
    path.join(process.cwd(), '../../apps/remix/public', normalizedRelativePath),
