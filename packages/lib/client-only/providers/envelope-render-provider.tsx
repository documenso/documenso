...
import { isEnvelopeSealed } from '../../utils/envelope';
...
export type EnvelopeRenderContent = Pick<
  EnvelopeContent,
  'id' | 'envelopeItemId' | 'metadata' | 'dataContentId' | 'zIndex'
>;
...
export const EnvelopeRenderProvider = ({
  ...
  const setZoom = useCallback((value: number) => {
    setZoomInternal(Math.min(ENVELOPE_VIEWER_MAX_ZOOM, Math.max(ENVELOPE_VIEWER_MIN_ZOOM, value)));
  }, []);
  ...
  const isContentsImprinted = version === 'current' && isEnvelopeSealed(envelope.status);
