...
export const parseSecondaryIdToDocumentId = (secondaryId: string) => {
  const value = parseInt(secondaryId.split('_')[1]);

  if (!secondaryId.startsWith(`${envelopeDocumentPrefixId}_`) || isNaN(value)) {
    throw new AppError(AppErrorCode.INVALID_BODY, {
      message: 'Secondary ID is not a document ID',
    });
  }
...
export const parseSecondaryIdToTemplateId = (secondaryId: string) => {
  const value = parseInt(secondaryId.split('_')[1]);

  if (!secondaryId.startsWith(`${envelopeTemplatePrefixId}_`) || isNaN(value)) {
    throw new AppError(AppErrorCode.INVALID_BODY, {
      message: 'Secondary ID is not a template ID',
    });
  }
...
export const buildDocumentId = (documentId: number) => {
  return `${envelopeDocumentPrefixId}_${documentId}`;
};
