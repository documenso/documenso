...
test('[ADMIN]: promote member to owner', async ({ page }) => {
  ...
  const adminMemberEmail = `admin-member-${nanoid()}@test.doku-seal.com`;
...
test('[ADMIN]: multiple promotions in sequence', async ({ page }) => {
  ...
  const member1Email = `member1-${nanoid()}@test.doku-seal.com`;
  const member2Email = `member2-${nanoid()}@test.doku-seal.com`;
