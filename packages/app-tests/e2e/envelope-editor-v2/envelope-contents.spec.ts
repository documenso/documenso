...
const getMeta = (metadata: unknown): Record<string, unknown> => {
  if (!isRecord(metadata)) {
    throw new Error('Content metadata is not an object');
  }
...
const assertAllContentTypesPersisted = async (surface: TEnvelopeEditorSurface, externalId: string) => {
  ...
  const types = envelope.contents.map((content) => getMeta(content.metadata).type).sort();

  expect(types).toEqual(['highlight', 'image', 'line', 'shape', 'text']);

  const shape = envelope.contents.find((content) => getMeta(content.metadata).type === 'shape');

  expect(getMeta(shape?.metadata).shape).toBe('rectangle');

  for (const content of envelope.contents) {
    const meta = getMeta(content.metadata);

    expect(meta.page).toBe(1);

    if (meta.type === 'line') {
      for (const key of ['startXPosition', 'endXPosition', 'startYPosition', 'endYPosition']) {
...
const assertSidebarSettingsPersisted = async (surface: TEnvelopeEditorSurface, externalId: string) => {
  ...
  const text = envelope.contents.find((content) => getMeta(content.metadata).type === 'text');
  const line = envelope.contents.find((content) => getMeta(content.metadata).type === 'line');
  const shape = envelope.contents.find((content) => getMeta(content.metadata).type === 'shape');

  expect(getMeta(text?.metadata)).toMatchObject({
    text: SIDEBAR_VALUES.text,
    fontSize: Number(SIDEBAR_VALUES.fontSize),
    textAlign: 'center',
  });

  expect(getMeta(line?.metadata)).toMatchObject({
    strokeWidth: Number(SIDEBAR_VALUES.strokeWidth),
    strokeStyle: 'dashed',
  });

  expect(typeof getMeta(shape?.metadata).fillColor).toBe('string');
};
...
const assertActionBarStylesPersisted = async (surface: TEnvelopeEditorSurface, externalId: string) => {
  ...
  expect(getMeta(shape?.metadata)).toMatchObject({
...
const assertResizePinnedToPage = async (surface: TEnvelopeEditorSurface, externalId: string) => {
  ...
  const meta = getMeta(envelope.contents[0].metadata);
...
const assertSelectedContentPersistedOnTop = async (surface: TEnvelopeEditorSurface, externalId: string) => {
  ...
  const [first, second] = [...envelope.contents].sort(
    (a, b) => Number(getMeta(a.metadata).positionX) - Number(getMeta(b.metadata).positionX),
  );
