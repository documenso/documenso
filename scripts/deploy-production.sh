...
CURRENT_TASK_DEFINITION=$(aws ecs describe-task-definition --task-definition "${APP_NAME}" --profile production --region "us-west-2")
...
')
NEW_TASK_INFO=$(aws ecs register-task-definition --region "us-west-2" --profile production --cli-input-json "$(echo "$NEW_TASK_DEFINITION" | jq -c)")    
