...
export default function OrganisationEmailSettingsPage({ params }: Route.ComponentProps) {
  ...
  const { data: emailDomainData, isLoading: isLoadingGroup } =
    trpc.organisation.emailDomain.find.useQuery(
      ...
      {
        enabled: !!organisation.id && !!emailDomainId,
      },
    );

  const emailDomain = emailDomainData?.data.find((g) => g.id === emailDomainId);
  ...
  const organisationMembersColumns = useMemo(() => {
    ...
    ] satisfies DataTableColumnDef<TFindOrganisationMembersResponse['data'][number]>[];
  }, [organisation]);
  ...
  return (
    <div>
      ...
      <div>
        <div className="mt-2 space-y-4 rounded border p-4">
          <div className="space-y-2">
            <Label>
              <Trans>Domain</Trans>
            </Label>

            <Input disabled value={emailDomain.domain} />
          </div>
