...
export const useEnvelopeCanvasContentsLayer = ({
  ...
  const { editorContents, selectedEditorTab } = useCurrentEnvelopeEditor();
    ...
    () =>
      sortContentsForRender(
        editorContents.localContents
          .filter(
            (content) =>
              (content.contentMeta.page ?? 1) === pageNumber && content.envelopeItemId === currentEnvelopeItem?.id,
          )
  ...
  /**
   * Write the new geometry of a content back into its metadata after a drag
  ...
  /**
   * Render every content on the page. Called when the page canvas is created.
   */
  /**
  ...
  useEffect(() => {
    ...
    // Only mirror while editable, otherwise a transformer would be attached
    // to an inert group.
    if (isEditable) {
      mirrorEditorSelectionToCanvas({
        layer,
