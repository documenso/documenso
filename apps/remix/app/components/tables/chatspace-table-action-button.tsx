...
import { NEXT_CHAT_URL } from '@documenso/lib/constants/app';
...
export type ChatTableActionButtonProps = {
  row: TDocumentRow;
};

export const ChatTableActionButton = ({ row }: ChatTableActionButtonProps) => {
  ...
  return match({
    ...
    .with({ isComplete: true }, () => (
      <Button className="w-32" asChild>
        <Link to={`${NEXT_CHAT_URL()}/newChat/${row.id}`} className="w-32">
          <Trans>Chat</Trans>
        </Link>
      </Button>
    ))
