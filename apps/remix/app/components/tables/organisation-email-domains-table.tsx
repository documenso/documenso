...
export const OrganisationEmailDomainsDataTable = () => {
  ...
  const { mutate: verifyEmails, isPending: isVerifyingEmails } =
    trpc.organisation.email.verify.useMutation({
      onSuccess: () => {
        toast({
          title: 'Emails verified',
          description: 'All emails have been verified successfully',
        });
      },
    });

  const { data, isLoading, isLoadingError } = trpc.organisation.email.find.useQuery(
    {
  ...
  const pendingEmails = results.data.filter((email) => email.status === EmailDomainStatus.PENDING);
  ...
  return (
    <>
      ...
      <AnimateGenericFadeInOut key={pendingEmails.length}>
        {pendingEmails.length > 0 && (
          ...
          >
            <div className="mb-4 sm:mb-0">
              <AlertTitle>
                <Trans>Verify domains</Trans>
              </AlertTitle>

              <AlertDescription className="mr-2">
                <Trans>This will re-verify all domains that are pending verification</Trans>
              </AlertDescription>
            </div>
