...
route.get(
  ...
  async (c) => {
    ...
    // The data content must be attached to a content of this envelope, so
    // knowing an ID alone grants nothing without access to its envelope.
    ...
    const { envelope } = dataContent.envelopeContent;
...
export const handleDataContentImageRequest = async ({
  ...
  // The file name rides along so the client can show it without another request.
  c.header('Content-Type', dataContent.metadata.mimeType);
  c.header('Content-Disposition', contentDisposition(dataContent.metadata.fileName, { type: 'inline' }));
