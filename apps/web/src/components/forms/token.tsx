...
export type HandleApiTokenFormReset = {
  reset: () => void;
};

const ApiTokenForm = forwardRef<HandleApiTokenFormReset, ApiTokenFormProps>(
  ({ teamId, className }: ApiTokenFormProps, ref) => {
    const router = useRouter();

    const [, copy] = useCopyToClipboard();
    const { toast } = useToast();

    const NEWLY_CREATED_TOKEN_DEFAULT = '';
    const NO_EXPIRATION_DATE_DEFAULT = false;
    const [newlyCreatedToken, setNewlyCreatedToken] = useState(NEWLY_CREATED_TOKEN_DEFAULT);
    const [noExpirationDate, setNoExpirationDate] = useState(NO_EXPIRATION_DATE_DEFAULT);
    ...
    useImperativeHandle(
      ...
      () => {
        return {
          reset() {
            form.reset();
            setNewlyCreatedToken(NEWLY_CREATED_TOKEN_DEFAULT);
            setNoExpirationDate(NO_EXPIRATION_DATE_DEFAULT);
          },
        };
      },
      [NEWLY_CREATED_TOKEN_DEFAULT, NO_EXPIRATION_DATE_DEFAULT, form],
    );
          ...
          <Card className="mt-8" gradient>
            <CardContent className="p-4">
              <p className="text-muted-foreground mt-2 text-sm">
                Your token was created successfully! Make sure to copy it because you won't be able
                to see it again!
              </p>
...
ApiTokenForm.displayName = 'API Token Form';
