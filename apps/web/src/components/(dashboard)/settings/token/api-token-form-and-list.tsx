...
import type { HandleApiTokenFormReset } from '~/components/forms/token';
import { ApiTokenForm } from '~/components/forms/token';

export type ApiTokenFormAndListProps = {
  tokens: {
    id: number;
    name: string;
    algorithm: 'SHA512';
...
export const ApiTokenFormAndList = ({ tokens }: ApiTokenFormAndListProps) => {
  const apiFormRef = useRef<HandleApiTokenFormReset>(null);
  const handleOnDelete = () => {
    apiFormRef?.current?.reset();
  };
  ...
  return (
    <div>
      ...
      <ApiTokenForm className="max-w-xl" ref={apiFormRef} />
          ...
          {tokens.map((token) => (
            <div key={token.id} className="border-border rounded-lg border p-4">
              <div className="flex items-center justify-between gap-x-4">
                ...
                <div>
                  <DeleteTokenDialog token={token} onDelete={handleOnDelete}>
