...
export const NewTemplateDialog = () => {
  ...
  const onSubmit = async (values: TCreateTemplateFormSchema) => {
    ...
    try {
      ...
      toast({
        title: `${t('template_document_uploaded')}`,
        description: `${t('redirect_to_the_template_page')}`,
        duration: 5000,
      });
  ...
  return (
    <Dialog open={showNewTemplateDialog} onOpenChange={setShowNewTemplateDialog}>
      ...
      <DialogContent className="w-full max-w-xl">
        <DialogHeader>
          <DialogTitle className="mb-4">{t('new_template')}</DialogTitle>
        </DialogHeader>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('name_your_template')}</FormLabel>
                    <FormControl>
                      <Input id="email" type="text" className="bg-background mt-1.5" {...field} />
                    </FormControl>
                    <FormDescription>
                      <span className="text-muted-foreground text-xs">{t('leave_this_empty')}</span>
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <div>
                <Label htmlFor="template">{t('upload_a_document')}</Label>
                  ...
                  {uploadedFile ? (
                    <Card gradient className="h-[40vh]">
                      <CardContent className="flex h-full flex-col items-center justify-center p-2">
                        ...
                        >
                          <X className="h-6 w-6" />
                          <span className="sr-only">{t('remove_template')}</span>
                        </button>
