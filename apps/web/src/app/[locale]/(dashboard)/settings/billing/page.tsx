...
export default async function BillingSettingsPage({ params: { locale } }: PageParams) {
  ...
  return (
    <div>
      <h3 className="text-2xl font-semibold">{t('billing')}</h3>

      <div className="text-muted-foreground mt-2 text-sm">
        {isMissingOrInactiveOrFreePlan && (
          <p>
            {t('you_are_currently')} <span className="font-semibold">{t('free_plan')}</span>.
          </p>
        )}
          ...
          match(subscription.status)
            .with('ACTIVE', () => (
              <p>
                {subscriptionProduct ? (
                  <span>
                    {t('you_are_currently_subscribe')}
                    <span className="font-semibold">{subscriptionProduct.name}</span>
                  </span>
                ) : (
                  <span>{t('currently_have_an_active_plan')}</span>
                )}
            ...
            .with('PAST_DUE', () => <p>{t('update_payment_information')}</p>)
            .otherwise(() => null)}
      </div>
