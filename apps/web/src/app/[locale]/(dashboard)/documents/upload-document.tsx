...
export const UploadDocument = ({ className }: UploadDocumentProps) => {
  ...
  const disabledMessage = useMemo(() => {
    if (remaining.documents === 0) {
      return `${t('reached_document_limit')}`;
    }

    if (!session?.user.emailVerified) {
      return `${t('verify_to_upload_document')}`;
    }
  ...
  const onFileDrop = async (file: File) => {
    try {
      ...
      toast({
        ...
        description: `${t('document_uploaded_successfully')}`,
        duration: 5000,
      });
      ...
      if (error instanceof TRPCClientError) {
        ...
        toast({
          title: `${t('error')}`,
          description: `${t('error_occured_while_uploading_document')}`,
          variant: 'destructive',
        });
      }
    } finally {
      ...
      <div className="absolute -bottom-6 right-0">
        {remaining.documents > 0 && Number.isFinite(remaining.documents) && (
          <p className="text-muted-foreground/60 text-xs">
            {t('document_remaining_this_month', {
        ...
        <div className="bg-background/60 absolute inset-0 flex items-center justify-center rounded-lg backdrop-blur-sm">
          <div className="text-center">
            ...
            <p className="text-muted-foreground/60 mt-2 text-sm">
              {t('upload_up_to', { quota: quota.documents })}
            </p>
