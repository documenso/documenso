...
export const NameField = ({ field, recipient }: NameFieldProps) => {
  ...
  const onSign = async (source: 'local' | 'provider' = 'provider') => {
    try {
      ...
      toast({
        title: `${t('error')}`,
        description: `${t('error_occured_while_signing_document')}`,
        variant: 'destructive',
      });
    }
  };
  ...
  const onRemove = async () => {
    try {
      ...
      toast({
        title: `${t('error')}`,
        description: `${t('error_occured_while_removing_document')}`,
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <SigningFieldContainer field={field} onSign={onSign} onRemove={onRemove} type="Name">
      ...
      <Dialog open={showFullNameModal} onOpenChange={setShowFullNameModal}>
        <DialogContent>
          <DialogTitle>
            {t('sign_as_recipient_name', { recipient_name: recipient.name })}{' '}
            <span className="text-muted-foreground">({recipient.email})</span>
          </DialogTitle>

          <div className="py-4">
            <Label htmlFor="signature">{t('full_name')}</Label>
