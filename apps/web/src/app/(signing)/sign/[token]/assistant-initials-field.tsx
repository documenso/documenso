...
export const AssistantInitialsField = ({
  field,
  onSignField,
  onUnsignField,
  selectedSigner,
}: AssistantInitialsFieldProps) => {
  ...
  const onSign = async () => {
    try {
      ...
      const initials = extractInitials(selectedSigner.name);
    ...
    <SigningFieldContainer field={field} onSign={onSign} onRemove={onRemove} type="Initials">
      ...
      {!field.inserted && (
        <p className="text-muted-foreground text-[clamp(0.425rem,25cqw,0.825rem)]">
          <Trans>Fill initials for: {selectedSigner?.name}</Trans>
        </p>
      )}
