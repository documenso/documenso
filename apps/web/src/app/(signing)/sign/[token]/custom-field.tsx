...
export const CustomField = ({ field, recipient }: NameFieldProps) => {
  const router = useRouter();

  const { toast } = useToast();

  const { customValue: providedCustomValue } = useRequiredSigningContext();
  ...
  const onSign = async () => {
    try {
      ...
      await signFieldWithToken({
        ...
        fieldId: field.id,
        value: providedCustomValue[field.id] ?? '',
        isBase64: false,
      });
