...
## Configuring Webhooks in Documenso
...
| `document.opened` | A recipient opens the document |
| `document.signed` | A recipient signs the document |
| `document.completed` | All recipients have signed the document |
| `document.rejected` | A recipient rejects the document |
| `document.cancelled` | The document owner cancels the document |

You can subscribe to all events or select specific ones based on your needs. For example, if you only need to know when documents are fully signed, subscribe only to `document.completed`.
...
3. The logs show each webhook call with:
...
Click on any individual call to see full details including headers and response data.

### Resending Failed Webhooks

If a webhook delivery fails, you can manually resend it:

1. Navigate to the webhook call details page
2. Click the **Resend** button
3. Documenso will attempt to deliver the same payload again
...
2. **Verify signatures**: Use a webhook secret and verify the `X-Documenso-Signature` header (see [Verification](/docs/developers/webhooks/verification))
3. **Validate payloads**: Validate incoming data before processing
4. **Respond quickly**: Return 200 OK immediately, then process asynchronously
