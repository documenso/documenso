...
Documenso sends webhook events when documents and recipients reach specific states in the signing workflow. Each event includes the full document payload with recipient information.
...
1. **Verify the signature** - Check the `X-Documenso-Secret` header matches your configured secret
2. **Check event type** - Use the `event` field to determine the action
3. **Process idempotently** - Webhooks may be retried, so handle duplicate events
4. **Respond quickly** - Return a 200 status code within 30 seconds
