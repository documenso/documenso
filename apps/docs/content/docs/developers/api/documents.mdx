...
Documents (called "envelopes" in the API) are the core resource in Documenso. You can create documents with recipients and fields, send them for signing, track their status, and retrieve the completed PDFs. Each document contains one or more PDF files, a list of recipients, and the fields they need to fill.
