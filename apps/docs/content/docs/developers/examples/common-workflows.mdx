...
<Steps>
  <Step>Build the recipient payload with signature and date fields</Step>
  <Step>
...
? [
{ identifier: 0, type: 'SIGNATURE', page: 1, positionX: 10, positionY: 80 - index * 10, width: 30, height: 5 },
{ identifier: 0, type: 'DATE', page: 1, positionX: 50, positionY: 80 - index * 10, width: 20, height: 3 },
]
: [],
}));
...
<Steps>
  <Step>Fetch the template to get recipient and field IDs</Step>
  <Step>
...
<Steps>
  <Step>Fetch the template and get the signer recipient slot ID</Step>
  <Step>
...
const template = await templateResponse.json();
const signerSlot = template.recipients.find((r: { role: string }) => r.role === 'SIGNER');
...
<Steps>
  <Step>Go to **Team Settings** → **Webhooks**</Step>
  <Step>Click **Create Webhook**</Step>
  <Step>
