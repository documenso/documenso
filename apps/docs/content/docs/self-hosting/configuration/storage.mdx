...
<Tabs items={['Advantages', 'Limitations']}>
  <Tab value="Advantages">
    - No external dependencies - Simple deployment - Automatic backups with database
  </Tab>
  <Tab value="Limitations">
    - Increases database size significantly - Slower for large files - Database backup/restore takes
    longer - Not recommended for files larger than 10MB
  </Tab>
</Tabs>
...
<Tabs items={['Nginx', 'S3', 'CloudFront']}>
  <Tab value="Nginx">Set `client_max_body_size` to match or exceed your upload limit.</Tab>
  <Tab value="S3">
    Default object size limit is 5GB; multipart upload may be required for large files.
  </Tab>
  <Tab value="CloudFront">Default limit is 50MB per request.</Tab>
</Tabs>
