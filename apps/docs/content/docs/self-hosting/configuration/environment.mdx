...
| `NEXT_PRIVATE_WEBHOOK_SSRF_BYPASS_HOSTS`  | Comma-separated hostnames or IPs whose webhook deliveries are allowed to resolve to private or loopback addresses (e.g., `internal.example.com,192.168.1.5`). Leave empty to block all private-address webhook targets. |                                  |
