...
This page provides a complete reference for all environment variables used to configure a self-hosted Documenso instance.
...
| ---------------------------------- | ----------------------------------------------------------------------------- | ------------ |
| `NEXT_PRIVATE_DATABASE_URL`        | PostgreSQL connection URL. Supports connection pooling (e.g., PgBouncer).     | Yes          |
| `NEXT_PRIVATE_DIRECT_DATABASE_URL` | Direct PostgreSQL URL for migrations. Required when using connection pooling. | When pooling |
...
| Variable                                | Description                                                                |
| --------------------------------------- | -------------------------------------------------------------------------- |
| `NEXTAUTH_SECRET`                       | Secret for NextAuth.js session encryption. Must be at least 32 characters. |
| `NEXT_PRIVATE_ENCRYPTION_KEY`           | Primary key for encrypting sensitive data. Must be at least 32 characters. |
| `NEXT_PRIVATE_ENCRYPTION_SECONDARY_KEY` | Secondary encryption key for key rotation. Must be at least 32 characters. |
...
| Variable                               | Description                               |
| -------------------------------------- | ----------------------------------------- |
| `NEXT_PRIVATE_MICROSOFT_CLIENT_ID`     | Microsoft/Azure AD application client ID. |
| `NEXT_PRIVATE_MICROSOFT_CLIENT_SECRET` | Microsoft/Azure AD client secret.         |
...
| `NEXT_PRIVATE_OIDC_PROVIDER_LABEL` | Label displayed on the OIDC sign-in button.         | `OIDC`  |
| `NEXT_PRIVATE_OIDC_SKIP_VERIFY`    | Skip email verification for OIDC accounts.          | `false` |
| `NEXT_PRIVATE_OIDC_PROMPT`         | OIDC prompt parameter. Set to empty string to omit. | `login` |
...
| `NEXT_PRIVATE_SMTP_SECURE`            | Force TLS connection (`true` or `false`). |
| `NEXT_PRIVATE_SMTP_UNSAFE_IGNORE_TLS` | Disable TLS entirely (not recommended).   |
...
| `NEXT_PRIVATE_SMTP_HOST`        | SMTP server hostname.                 |
| `NEXT_PRIVATE_SMTP_PORT`        | SMTP server port.                     |
| `NEXT_PRIVATE_SMTP_APIKEY_USER` | API key user for SMTP authentication. |
...
| `NEXT_PRIVATE_MAILCHANNELS_ENDPOINT`         | Custom MailChannels API endpoint (for proxies). |
...
| Variable                     | Description                          | Default |
| ---------------------------- | ------------------------------------ | ------- |
| `NEXT_PRIVATE_JOBS_PROVIDER` | Jobs provider: `local` or `inngest`. | `local` |
