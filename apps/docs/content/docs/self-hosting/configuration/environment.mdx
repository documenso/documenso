...
| Variable                              | Description                                                                         | Default |
| ------------------------------------- | ----------------------------------------------------------------------------------- | ------- |
| `DISABLE_SIGNUP`                      | Master switch. Disable all signup methods application-wide                          | `false` |
| `NEXT_PUBLIC_DISABLE_SIGNUP`          | Legacy alias for `DISABLE_SIGNUP`. Either being `true` activates the master switch  | `false` |
| `DISABLE_EMAIL_PASSWORD_SIGNUP`       | Disable email/password signup only. SSO signup is unaffected                        | `false` |
| `DISABLE_GOOGLE_SIGNUP`               | Block new accounts via Google. Existing Google-linked users can still sign in       | `false` |
| `DISABLE_MICROSOFT_SIGNUP`            | Block new accounts via Microsoft. Existing linked users can still sign in           | `false` |
| `DISABLE_OIDC_SIGNUP`                 | Block new accounts via OIDC, including the organisation portal                      | `false` |
...
- **`DISABLE_SIGNUP`** (master switch): Set to `true` to block all new signups across every method (email/password, Google, Microsoft, OIDC). When set, this also blocks new-account creation through the organisation OIDC authentication portal.
- **`NEXT_PUBLIC_DISABLE_SIGNUP`**: Legacy alias for `DISABLE_SIGNUP`, kept for backward compatibility. Setting either variable to `true` activates the master switch.
- **`DISABLE_EMAIL_PASSWORD_SIGNUP`**: Set to `true` to disable email/password signup only. SSO signup is still allowed.
- **`DISABLE_GOOGLE_SIGNUP`**, **`DISABLE_MICROSOFT_SIGNUP`**, **`DISABLE_OIDC_SIGNUP`**: Set to `true` to block brand-new account creation through the matching SSO provider. Existing users with the provider already linked can still sign in, and existing users can still link the provider to their account. `DISABLE_OIDC_SIGNUP` also blocks new-account creation through the organisation authentication portal.
...
The `DISABLE_*` variables are server-only (no `NEXT_PUBLIC_` prefix). Sign-in for existing users is never affected — only the creation of brand-new accounts.
...
DISABLE_EMAIL_PASSWORD_SIGNUP="true"
DISABLE_GOOGLE_SIGNUP="true"
DISABLE_MICROSOFT_SIGNUP="true"
