...
| `NEXT_PRIVATE_SIGNUP_INVITE_SECRET`          | Shared secret for the internal signup invite API                                      |         |
| `NEXT_PRIVATE_SIGNUP_INVITE_EXPIRY_DAYS`     | Default number of days before a signup invite expires (max 30)                          | `7`     |
...
- **`NEXT_PRIVATE_SIGNUP_INVITE_SECRET`**: Enables the internal signup invite API at `/api/internal/signup-invites`. When set together with `NEXT_PUBLIC_DISABLE_SIGNUP="true"`, invited users can still create email/password accounts. See [Signup Invites](/docs/self-hosting/configuration/signup-invites).
- **`NEXT_PRIVATE_SIGNUP_INVITE_EXPIRY_DAYS`**: Default invite lifetime in days. Can be overridden per invite when creating via the API.
