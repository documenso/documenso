...
| `NEXT_PRIVATE_SIGNING_TIMESTAMP_AUTHORITY`     | Comma-separated TSA URLs. Used for long-term archive timestamps when the TSP itself does not expose `signatures/timestamp`.       |         |
...
Long-term archive timestamps (PAdES B-LTA) are required for AES/QES envelopes. Resolution order at seal time:

1. If the TSP advertises `signatures/timestamp` in its `info` response, that endpoint is used.
2. Otherwise, the first URL in `NEXT_PRIVATE_SIGNING_TIMESTAMP_AUTHORITY` is used.
3. If neither is available, the instance fails at envelope completion (`CSC_PROVIDER_NO_TSA`).
...
| `CSC_PROVIDER_NO_TSA`      | TSP exposes no TSA and no env override is set    | Operator: configure `NEXT_PRIVATE_SIGNING_TIMESTAMP_AUTHORITY`          |
