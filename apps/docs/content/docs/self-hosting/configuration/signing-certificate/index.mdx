...
<Cards>
  ...
  <Card
    title="AWS KMS"
    description="Hardware-backed key protection with AWS Key Management Service."
    href="/docs/self-hosting/configuration/signing-certificate/aws-kms"
  />
  <Card
...
<Tabs items={['Self-Signed', 'CA-Issued', 'Google Cloud HSM', 'AWS KMS']}>
...
For organisations running on AWS, Documenso supports AWS KMS. This provides:

- FIPS 140-2-validated key storage
- Keys never leave KMS — only the digest is sent for signing
- Native AWS IAM integration (no separate GCP project required)
- CloudTrail audit logging of all cryptographic operations

See [AWS KMS](/docs/self-hosting/configuration/signing-certificate/aws-kms) for setup instructions.
