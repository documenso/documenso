...
Create an HSM key ring and an asymmetric signing key in Cloud KMS (see "[Creating an HSM Key](/docs/self-hosting/configuration/signing-certificate#creating-an-hsm-key)" below).
...
<Accordions type="multiple">
  <Accordion title="Failed to get private key bags">
    Cause:
    - Certificate has no password or incompatible encryption

    Solution:
    - Verify the certificate has a password set
    - Re-create with compatible encryption:

    ```bash
    openssl pkcs12 -export -out new-certificate.p12 -inkey private.key -in certificate.crt -keypbe PBE-SHA1-3DES -certpbe PBE-SHA1-3DES -macalg sha1
    ```
