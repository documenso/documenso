...
# Master switch — disables every signup method (legacy alias: NEXT_PUBLIC_DISABLE_SIGNUP)
...
<Callout type="info">
  ...
  setting `DISABLE_SIGNUP=true` (or, for backward compatibility,
  `NEXT_PUBLIC_DISABLE_SIGNUP=true`). For finer control, use the per-method switches
  `DISABLE_EMAIL_PASSWORD_SIGNUP`, `DISABLE_GOOGLE_SIGNUP`, `DISABLE_MICROSOFT_SIGNUP`,
  `DISABLE_OIDC_SIGNUP`, or restrict signups to specific email domains with
