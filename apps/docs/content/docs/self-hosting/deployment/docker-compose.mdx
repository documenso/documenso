...
This guide covers deploying Documenso with Docker Compose, including PostgreSQL and the Documenso application. This setup is production-ready and suitable for most self-hosted deployments.

For a standalone Docker container (when you manage the database separately), see [Docker Deployment](/docs/self-hosting/deployment/docker).
