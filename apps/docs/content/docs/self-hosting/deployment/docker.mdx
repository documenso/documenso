---
title: Docker
description: Single container Docker deployment.
---
...
This guide covers running Documenso as a standalone Docker container. Use this approach when you want to manage the database and other services separately, or when deploying to container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.

For a complete setup including PostgreSQL, see [Docker Compose Deployment](/docs/self-hosting/deployment/docker-compose).
...
| `NEXT_PRIVATE_SMTP_TRANSPORT`           | Email transport: `smtp-auth`, `smtp-api`, `resend`, or `mailchannels`       |
...
| Variable                           | Description                                                                           |
| ---------------------------------- | ------------------------------------------------------------------------------------- |
| `NEXT_PRIVATE_DATABASE_URL`        | Pooled connection for application queries                                             |
| `NEXT_PRIVATE_DIRECT_DATABASE_URL` | Direct connection for migrations (defaults to `NEXT_PRIVATE_DATABASE_URL` if not set) |
...
| `NEXT_PRIVATE_SMTP_PORT`     | SMTP server port (usually 587 or 465) |
| `NEXT_PRIVATE_SMTP_USERNAME` | SMTP username                         |
| `NEXT_PRIVATE_SMTP_PASSWORD` | SMTP password                         |
| `NEXT_PRIVATE_SMTP_SECURE`   | Set to `true` to force TLS            |
