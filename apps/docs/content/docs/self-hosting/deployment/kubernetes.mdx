...
| Probe Type       | Purpose                          | Endpoint      |
| ---------------- | -------------------------------- | ------------- |
| `livenessProbe`  | Restart container if unhealthy   | `/api/health` |
| `readinessProbe` | Remove from service if not ready | `/api/health` |
| `startupProbe`   | Allow time for initial startup   | `/api/health` |

### Probe Configuration

Adjust probe timings based on your environment:
...
```bash
# Pod resource usage
kubectl top pods -n documenso

# Deployment status
kubectl get deployment documenso -n documenso

# HPA status
kubectl get hpa documenso -n documenso
```
...
# Specific pod
kubectl logs documenso-<pod-id> -n documenso -f

# Previous container (if restarted)
kubectl logs documenso-<pod-id> -n documenso --previous
```
...
<Accordions type="multiple">
  <Accordion title="Pods not starting">
    Check pod status: `kubectl get pods -n documenso` and `kubectl describe pod <pod-name> -n documenso`. Common causes: ImagePullBackOff (check image name and registry access), CrashLoopBackOff (check logs for application errors), Pending (check resource requests and node capacity).
  </Accordion>
  <Accordion title="Database connection errors">
    Verify the database is accessible from the cluster: `kubectl run postgres-test --rm -it --image=postgres:15-alpine -n documenso -- psql "postgresql://user:password@host:5432/database" -c "SELECT 1"`.
  </Accordion>
  <Accordion title="Ingress not working">
    Check Ingress status: `kubectl get ingress -n documenso` and `kubectl describe ingress documenso -n documenso`. Verify the Ingress controller is running: `kubectl get pods -n ingress-nginx` or `kubectl get pods -n traefik`.
  </Accordion>
  <Accordion title="Certificate errors">
    Check the signing certificate secret: `kubectl get secret documenso-signing-cert -n documenso -o yaml`. Verify the certificate is mounted correctly: `kubectl exec -it <pod-name> -n documenso -- ls -la /opt/documenso/`.
  </Accordion>
  <Accordion title="Health check failures">
    Test the health endpoint: `kubectl port-forward svc/documenso 3000:80 -n documenso` then `curl http://localhost:3000/api/health`.
  </Accordion>
  <Accordion title="Rolling back a deployment">
    If a deployment fails, rollback: `kubectl rollout history deployment/documenso -n documenso` to view history, `kubectl rollout undo deployment/documenso -n documenso` for previous version, or `--to-revision=2` for a specific revision.
  </Accordion>
</Accordions>
