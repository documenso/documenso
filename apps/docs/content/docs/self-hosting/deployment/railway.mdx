...
Railway is a cloud platform that simplifies deploying and managing applications. It provides automatic builds from Git, managed PostgreSQL databases, and built-in networking with minimal configuration.

This guide covers deploying Documenso on Railway using either the one-click template or manual setup.
...
Click "Deploy" to start the deployment. Railway builds the application from the Dockerfile and provisions the database. This takes approximately 5-10 minutes for the initial build.
...
Once deployed, Railway provides a URL like `documenso-production.up.railway.app`. Click the URL in the Railway dashboard to access your Documenso instance.
...
5. Fork or select the [Documenso repository](https://github.com/documenso/documenso)
...
2. Select "Database"
3. Choose "PostgreSQL"
4. Railway provisions the database and automatically sets `DATABASE_URL`
...
3. Use the provided connection string with your preferred PostgreSQL client
...
1. Click on your Documenso service
2. Go to "Settings" > "Networking"
3. Click "Generate Domain" to get a Railway subdomain, or proceed to add a custom domain
...
2. Enter your domain (e.g., `sign.yourdomain.com`)
...
Railway automatically scales within your configured limits based on demand.
...
Check the build logs in Railway for specific errors. Common causes:

- **Out of memory**: Increase the build memory limit in service settings
- **Docker build timeout**: Railway has a 20-minute build timeout; ensure your build completes within this limit
...
2. Go to "Deployments"
3. Click on the failed deployment to view logs

Common issues:

- **Missing environment variables**: Verify all required variables are set
- **Database connection failed**: Check `NEXT_PRIVATE_DATABASE_URL` references the correct PostgreSQL service
- **Invalid encryption keys**: Ensure keys are at least 32 characters

### Emails not sending

Verify SMTP configuration:

- Check `NEXT_PRIVATE_SMTP_TRANSPORT` is set to `smtp-auth`
- Verify SMTP credentials are correct
- Ensure your SMTP provider allows connections from Railway's IP addresses

### Database connection errors

- Verify PostgreSQL service is running
- Check the variable reference syntax: `${{Postgres.DATABASE_URL}}`
- Try redeploying the application

### Slow performance

- Increase memory allocation for the service
- Check database query performance in Railway's metrics
- Consider upgrading to a larger PostgreSQL instance
