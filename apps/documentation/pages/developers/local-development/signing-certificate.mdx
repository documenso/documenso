...
If you are using Docker, you may run into permission issues when the application tries to sign a document using your certificate.

If this happens, you will need to ensure the certificate has correct permission so the application can access it.

This can be done by using `chown` to allow the NextJS user (1001) to read the certificate.
