...
Before adding fields to a document, you need each recipients' ID. If the document already has recipients, you can query the document to retrieve the recipients details. If the document doesn't have any recipients, you need to add a recipient via the UI or API before adding a field.

<Steps>

### Retrieve the Recipient(s) ID(s)

Perform a `GET` request to the `/api/v1/documents/{id}` to retrieve the details of a specific document, including the recipients information.

An example reponse would look like this:
...
If the document doesn't already have recipient(s), you can add a recipient via the API. Make a `POST` request to the `/api/v1/documents/{documentId}/recipients` endpoint with the recipient information. This endpoint takes the following JSON payload:
