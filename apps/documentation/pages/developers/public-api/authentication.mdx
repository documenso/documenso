...
description: Learn how to create a Doku-Seal API key and authenticate your API requests.
---

# API Authentication

Doku-Seal uses API keys for authentication. An API key is a unique token that is generated for each client. The client must provide the key whenever it makes an API call. This way, Doku-Seal can identify the clients making the requests and authorize their access to the API.
...
![A screenshot of the Doku-Seal's dashboard that shows the dropdown menu when you click on your user avatar](/public-api-images/doku-seal-user-dropdown-menu.webp)
...
![A screenshot of the Doku-Seal's user settings page that shows the API Tokens page](/public-api-images/api-tokens-page-doku-seal.webp)
...
![A screenshot of the newly created API token in the Doku-Seal dashboard](/public-api-images/doku-seal-api-key-blurred.webp)

Once you've created the token, Doku-Seal will display the key on the screen. Make sure to copy the key and store it securely. You won't be able to see the key again once you refresh/leave the page.
...
```bash
curl --location 'https://app.doku-seal.com/api/v2/envelope/create' \
