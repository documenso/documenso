---
title: How to Self-Host Doku-Seal
description: Learn how to self-host Doku-Seal on your server or cloud infrastructure using Docker or other deployment methods.
---
...
The following guide will walk you through setting up Doku-Seal manually on your server or cloud infrastructure.
...
Navigate to the `doku-seal` folder and create a `.env` file from the example `.env.example` file:
...
<Callout type="info">
  If you use a reverse proxy in front of Doku-Seal, don't forget to provide the public URL for the
...
The following guide will walk you through setting up Doku-Seal using Docker. You can choose between a Docker Compose production setup or a standalone container.

We provide a Docker container for Doku-Seal, published on both DockerHub and GitHub Container Registry.

- [DockerHub](https://hub.docker.com/r/doku-seal/doku-seal)
- [GitHub Container Registry](https://ghcr.io/doku-seal/doku-seal)
...
This setup includes a PostgreSQL database and the Doku-Seal application. You will need to provide your own SMTP details using environment variables.

<Steps>

### Download the Docker Compose File

Download the Docker Compose file from the Doku-Seal repository - [compose.yml](https://raw.githubusercontent.com/doku-seal/doku-seal/release/docker/production/compose.yml).
...
4. **Add certificate configuration** to your `.env` file:
   ```bash
   NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH=/opt/doku-seal/cert.p12
...
The command will start the PostgreSQL database and the Doku-Seal application containers.

### Access the Application

Access the Doku-Seal application by visiting `http://localhost:3000` in your web browser.

</Steps>

### Option 2: Standalone Docker Container

If you prefer to host the Doku-Seal application on a specific container provider, use the pre-built Docker image from DockerHub or GitHub's Package Registry. You will need to provide your own database and SMTP host.

<Steps>

### Pull the Docker Image

Pull the Doku-Seal Docker image from DockerHub:
...
docker run -d \
 ...
 -v /path/to/your/keyfile.p12:/opt/doku-seal/cert.p12
 doku-seal/doku-seal
```
...
You can access the Doku-Seal application by visiting the URL you provided for the `NEXT_PUBLIC_WEBAPP_URL` environment variable in your web browser.

</Steps>

### Advanced Configuration

The environment variables listed above are a subset of those available for configuring Doku-Seal. The table below provides a complete list of environment variables and their descriptions.

| Variable                                     | Description                                                                                         |
| -------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `PORT`                                       | The port on which the Doku-Seal application runs. It defaults to `3000`.                            |
...
We offer several alternative deployment methods for Doku-Seal if you need more options.
