...
The `cert.p12` file is required to sign and encrypt documents. You have two options:

#### Option A: Use the Automated Setup Script (Recommended)

Download and run our setup script that handles certificate configuration automatically:

```bash
curl -fsSL https://raw.githubusercontent.com/documenso/documenso/main/docker/setup-selfhost.sh | bash
```

Or download and run separately:

```bash
curl -o setup-selfhost.sh https://raw.githubusercontent.com/documenso/documenso/main/docker/setup-selfhost.sh
chmod +x setup-selfhost.sh
./setup-selfhost.sh
```
...
- **Certificate setup options:**
  - Use existing .p12 certificate file (host-based)
  - Create certificate inside Docker container (recommended - avoids permission issues)
  - Generate self-signed certificate (host-based)
- Environment variable configuration
- Docker Compose file generation with proper volume configuration
- Proper file permissions and ownership

**Container-based certificates** are recommended as they:

- Avoid file permission issues between host and container
- Use Docker volumes for persistence
- Create certificates with compatible formats (PBE-SHA1-3DES)
- Simplify deployment and maintenance

#### Option B: Manual Certificate Setup

If you prefer to set up manually, you can choose between host-based or container-based certificates:

**Host-based Certificate Setup (Traditional):**

1. **Place your certificate file** in a accessible location on your host system
...
4. **Add certificate configuration** to your `.env` file:
   ...
   NEXT_PRIVATE_SIGNING_PASSPHRASE=your_passphrase_if_any
   ```

**Container-based Certificate Setup (Recommended):**

1. **Use Docker volumes** in your `compose.yml` file:
...
2. **Create certificate inside container** after starting services:

   ```bash
   # Start the services first
   docker-compose up -d

   # Create certificate inside container
   docker-compose exec --user root documenso sh -c "
     ...
     # Generate certificate (adjust subject as needed)
     openssl req -new -x509 -key private.key -out certificate.crt -days 365

     # Create PKCS12 with compatible format
   ...
   # Restart to load certificate
   docker-compose restart documenso
   ```

3. **Add certificate configuration** to your `.env` file:
   ```bash
...
<Callout type="info">
  **Alternative:** Instead of mounting a file, you can provide the certificate as a base64-encoded
  environment variable: ```bash NEXT_PRIVATE_SIGNING_LOCAL_FILE_CONTENTS=base64_encoded_cert_content
  ``` This approach avoids file permission issues entirely.
</Callout>
