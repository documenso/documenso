...
In addition to embedding signing experiences, Doku-Seal now supports embedded authoring, allowing you to integrate document and template creation directly within your application.
...
```jsx
import { unstable_EmbedCreateDocument as EmbedCreateDocument } from '@doku-seal/embed-react';
...
This API endpoint requires authentication with your Doku-Seal API key. The token has a default expiration of 1 hour, but you can customize this duration based on your security requirements.

You can find more details on this request at our [API Documentation](https://openapi.doku-seal.com/reference#tag/embedding)
...
| `host`             | string  | Optional custom host URL. Defaults to `https://app.doku-seal.com`. |
