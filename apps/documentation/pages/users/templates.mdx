...
You can add placeholders for the template recipients. Placeholders specify where the recipient's name, email, and other information should be placed in the document.
...
If you add placeholders to the template, you must replace them with actual recipients when creating a document from the template. See the modal from [step 6](#use-a-template).
...
You can also send the document straight to the recipients for signing by checking the "Send document" checkbox. This will send the document to the recipients for signing.
