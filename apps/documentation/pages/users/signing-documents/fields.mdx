...
- The text field characters count can't be higher than the character limit.
...
- If a number format is set, the value must match the number format.
...
The recipient can modify the number field value since the field is not read-only. To change the value, the recipient must click the field to un-sign it.
...
Since the number field has a validation rule set, the recipient needs to enter a value that meets the rules. In this example, the value needs to be between 5 and 50. Otherwise, an error message will appear, and the field will not be signed.
