...
The guide assumes you have a Documenso account. If you don't have one yet, you can create a free account [here](https://documen.so/free-docs).
...
After the upload is complete, you will be redirected to the document's page. Here you can configure the document's settings, add recipients and fields.
...
Click on the "+ Add Signer" button to add a new recipient. On this page, you can configure the recipient's email address, name, role, and authentication method.
...
Documens supports 9 different field types that can be added to the document. Each field type collects different information from the recipients when they sign the document.
...
Signer Roles require at least 1 signature field. If you try to send a document without a signature field, you will get an error message.
...
Before sending the document, you can configure the email settings. You can customize the email subject, message, and sender name.
