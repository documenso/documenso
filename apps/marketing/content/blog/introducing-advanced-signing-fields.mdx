...
The introduction of these new fields brings more flexibility and variety to Documenso. As the document owner, they enable you to gather more specific or extra information from the signers.
...
The fourth field, "text-3", has the same configurations as the second one, with one addition—`read-only` is checked. That means the field auto-signs with the default value, and you cannot modify it.
...
The only exception is the fourth field, a read-only field, which cannot be unsigned or modified.
...
It's important to mention that the image is slightly modified so you can see both states of the field — unsigned and signed. Since the field has a checked value (option `radio-val-2-checked`), it will automatically sign with that value and appear like the field marked with the number 1.

If the field is not marked as read-only, the signer can unsign it and choose another option by clicking on it. Alternatively, you can re-sign with the default value by refreshing the page when the field is unsigned.
...
When configuring the "Dropdown/Select" field, you can:
...
The "Dropdown/Select" field will not auto-sign if you don't set a default value. The signer must click on the field and select an option from the dropdown list to sign it. After signing, the field will display the selected value, similar to a signed text field.

When the field is marked as required, signers must select a value before completing the signing process. However, if you mark the field as read-only, signers can view the selected value but not modify it.
...
No validation rule has been set in this example, allowing the signer to select any options. However, if a validation rule were applied, the signer's choices would need to meet the validation criteria.
...
Initially, we considered creating a new database table called `FieldMeta` to store the information for the new advanced fields, with each property represented as a separate column in the table. However, this approach has 2 issues.
...
This field can store any JSON object, allowing us to store each field's settings. However, this introduces a potential issue — we lose the benefit of type checks that come with using specific database field types.
...
Currently, the app saves the fields and their associated settings to the database when the user advances to the next step.
...
In this case, the solution is to temporarily store the advanced settings in local storage. This is because the fields are available only locally. If the fields were saved in the database, we could also save the advanced settings along with them.
...
The Radio and Checkbox field implementations were challenging from both the logical and design standpoints. The Radio field can contain empty and non-empty values, and the Checkbox field allows the user to select multiple values.
...
In contrast to the other challenges, I will discuss the general implementation rather than a specific solution. We'll focus on the particularities and the most challenging parts of the implementation rather than going over the entire implementation. I'll include a link to the complete implementation for each field so you can check it out if you're interested.
...
The value is saved in the database whenever the field is signed, whether by auto-signing or by the user. Similarly, the value is removed from the database when the field is unsigned.
...
While the implementation is more complex, we'll focus on its most challenging aspects.

At its core, the app allows signers to select multiple options, with the field automatically signing based on these selections. The signers can un-sign the field by deselecting options or clearing all the selections. Throughout this process, the system enforces validation rules, ensuring that the signers select the required number of options to sign the field successfully.
...
The document editor consists of various components, including those representing the fields, recipients, and other elements. That means the same colours and code are reused across multiple components.
...
We moved the colours and associated hooks to a separate file to address the challenge of reusing colours across multiple components. The styles would only be defined in this file and accessed from the components through custom hooks.
...
This approach makes managing the colours and styles easier, as they are defined in a single file. If we need to change or add new colours, we can do so in a single place. This approach also makes the code more modular and reusable, as the styles are decoupled from the components.

Moreover, we opted for CSS variables to define the colours as they allow for more flexibility and consistency in styling. This way, we define a single CSS variable for each colour that can cover a wide range of states without relying on the multitude of TailwindCSS classes. For example, you can easily set the opacity and lightness of a colour without having to use multiple classes. The CSS variables help align the colours with our brand guidelines while simplifying the overall styling process.
