...
<figure>
  ...
  <figcaption className="text-center">
    The next generation of Notario and signing infrastructure.
  </figcaption>
</figure>

> TLDR; We're rewriting Notario to move on from our MVP foundations and create an even better base for the project. This rewrite will provide us the opportunity to fix a few things within the project while enabling a faster development process moving forward.

# Introduction

At Notario, we're building the next generation of signing infrastructure with a focus on making it inclusive and accessible for all. To do this we need to ensure that the software we write is also inclusive and accessible and for this reason we’ve decided to take a step back and perform a _quick_ rewrite.
...
Fortunately, this wasn't a case of someone joining the team and proposing a rewrite due to a lack of understanding of the codebase and context surrounding it. Prior to joining Notario as a co-founder, I had spent an extensive amount of time within the Notario codebase and had a fairly intimate understanding of what was happening for the most part. This knowledge allowed me to make the fair and simultaneously hard call to take a quick pause so we can rebuild our current foundations to enable accessibility and a faster delivery time in the future.

# The Reasoning: TypeScript

Our primary reason for the rewrite is to better leverage the tools and technologies we've already chosen, namely TypeScript. While Notario currently uses TypeScript, it's not fully taking advantage of its safety features, such as generics and type guards.
...
- Remove any features that were in the older version of Notario, such as free signatures (signatures that have no corresponding field).
