 ...
 *
 * Usage: npx ts-node security-hardening.checklist.ts
 *
 * Each test must PASS before the PR is submitted.
 * Any FAIL is a blocking bug — fix before D8.
 */

import { isPDF, assertFileSize, zeroBuf, hashIP, checkRateLimit } from "./utils/verify-utils";
...
test("Memory zeroing: buffer is all zeros after zeroBuf()", () => {
  ...
  return allZero ? true : `Buffer not zeroed: first non-zero byte at position ${buf.indexOf(buf.find(b => b !== 0)!)}`;
});
...
test("Memory zeroing: fires even when isPDF throws (try/finally simulation)", () => {
  ...
  try {
    if (!isPDF(buf)) throw new Error("Not a PDF");
  } finally {
    zeroBuf(buf);
    zeroingFired = true;
  }
...
test("IP hashing: unknown/missing IP handled without throwing", () => {
  try { hashIP("unknown"); return true; } catch { return false; }
});
...
test("Rate limiting: first 100 requests allowed", () => {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    if (!allowed) { failedBefore100 = true; break; }
  }
...
test("Rate limiting: 101st request blocked (returns allowed: false)", () => {
  ...
  for (let i = 0; i < 100; i++) checkRateLimit(testIP);
...
console.log(`\n────────────────────────────────────────────────`);
console.log(` ${passed}/${total} passed   ${failed > 0 ? `\x1b[31m${failed} FAILED — blocking bugs\x1b[0m` : "\x1b[32mAll clear\x1b[0m"}`);
console.log(`────────────────────────────────────────────────\n`);
